"""
Last Letter Concatenation baseline eval via Doubleword batch — Qwen3.5-397B.

Writes under experiments/doubleword_batches/last_letter_qwen/.
No /no_think prefix (thinking on by default).

Usage (repo root):
    .venv/bin/python src/doubledword/last_letter_qwen_baseline_eval.py --n 10 --window 1h
    .venv/bin/python src/doubledword/last_letter_qwen_baseline_eval.py --window 24h
    .venv/bin/python src/doubledword/last_letter_qwen_baseline_eval.py --batch-id <uuid>
"""
...
def main():
    p = argparse.ArgumentParser(description="Last Letter baseline eval — Qwen3.5-397B")
    ...
    run_smoke_test(
        ...
        n=args.n,
        input_path="data/baseline/last_letter_baseline.csv",
        ...
        label="last_letter_baseline_eval",
    )
