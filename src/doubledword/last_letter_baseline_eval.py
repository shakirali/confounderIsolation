"""
Last Letter Concatenation baseline eval via Doubleword batch.

Writes under experiments/doubleword_batches/last_letter/.
Eval model: Nemotron 120B. No /no_think prefix.

Usage (repo root):
    .venv/bin/python src/doubledword/last_letter_baseline_eval.py --n 10 --window 1h
    .venv/bin/python src/doubledword/last_letter_baseline_eval.py --window 24h
    .venv/bin/python src/doubledword/last_letter_baseline_eval.py --batch-id <uuid>
"""
...
def main():
    p = argparse.ArgumentParser(description="Last Letter Concatenation baseline Doubleword batch eval")
